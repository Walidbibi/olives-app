...
- [ ] **Onglet Traitement** (session 2026-05-16) : Nouvel onglet dédié affichant tous les traitements réalisés (taille, labour, engrais, salaires laboureurs), triés par date et filtrables par parcelle. Source de données : charges de type `traitement_oliviers`. ⚠️ Point à trancher : la table `charge` n'a pas de colonne `parcelle_id` — pour le "par parcelle", il faudra soit ajouter ce champ à la table (modification DB), soit se contenter d'un filtre par date/sous-type sans lien parcelle. Navigation : nouvel onglet `traitement` dans App.jsx.
